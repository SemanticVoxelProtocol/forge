...
`packages/core/hash.ts` 提供了 `computeHash`、`hashL3`、`hashL4`、`hashL5`、`hashL2` 等函数。rehash 后 `forge check` 就能通过 hash 一致性检查。
...
```bash
forge init --name my-app --intent "做什么" --host <host>
```
...
forge view l4            # 所有 L4 流程概览
forge view l4 order-flow # L4 流程详情
forge view l3            # 所有 L3 块概览
forge view l3 greet      # L3 块详情
forge view l2            # 所有 L2 代码块
```
Overview 模式把每个实体压缩成一行，Detail 模式展示完整信息。视图由 `packages/core/view.ts` 的纯函数渲染，支持 i18n。

### 6.4 forge compile-plan
生成编译任务清单：
```bash
...
重算哈希值：
```bash
forge rehash l3          # 重算所有 L3 的 contentHash
forge rehash l3/greet    # 重算指定 L3
forge rehash l4          # 重算所有 L4
forge rehash l5          # 重算 L5
```

### 6.7 forge link
创建/更新 L2 映射：
```bash
forge link greet --files src/greet.ts
```

### 6.8 forge compile-blueprint
编译 svp-blueprint 节点格式（YAML → JSON）。

### 6.9 forge blueprint
生成自包含的 HTML 可视化节点图查看器：
```bash
forge blueprint                     # 当前目录，打开浏览器
forge blueprint -r examples/order-service   # 指定项目
forge blueprint -o blueprint.html   # 输出到文件
```
...
通用规则强制执行 SVP 的核心不变量：
- 严格向下只写——不修改上层
- 使用 `forge rehash` 修复 contentHash
- 使用 `forge link` 创建 L2 映射
- 保持实现最小化——满足契约，仅此而已
...
这意味着：如果你的 L5 设置了 `language: "zh"`，整个工具链——从 `forge check` 的错误报告到 `forge view` 的数据展示再到 AI prompt 中的语言指令——都会切换为中文。
...
`forge prompt compile/recompile/review` 时会自动加载并注入 prompt，让 AI 在编译时能获得更多上下文。
...
```bash
forge init --name my-app --intent "做什么" --host <host>
forge prompt design-l5 --intent "详细描述系统"
...
### 阶段二：设计流程（人）
```bash
forge prompt design-l4 --intent "描述流程" --kind flow
...
### 阶段三：设计契约（人 + AI）
```bash
forge prompt design-l3 validate-order --flow order-flow --step 0 --intent "校验订单"
...
### 阶段四：编译代码（AI）
```bash
forge compile-plan           # 查看待编译任务
forge prompt compile greet   # 生成 compile prompt
# AI 生成 L1 代码
forge link greet --files src/greet.ts
forge rehash l2
forge check  # 全部通过 ✓
...
forge compile-plan  # 自动检测到 recompile 任务
