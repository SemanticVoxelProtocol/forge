...
export function buildScanL3Prompt(input: ScanL3Input): string {
  ...
  return (
    ...
    [
      ...
      "- Record `evidence`, `confidence`, `assumptions`, and `needsHumanReview` for each governed file. SVP is language-agnostic, so you must leave source evidence for your language-specific judgment.",
      ...
      "- Fill `observedSignature` from the source when visible and `contractSignature` as a language-neutral semantic signature.",
      "- Record source evidence for the declaration or nearest stable excerpt plus confidence and assumptions.",
      ...
      "- Do not add language-specific parser assumptions to SVP artifacts; if uncertain, lower confidence or set `needsHumanReview: true`",
