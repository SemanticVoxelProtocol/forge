...
**Subagent 派发**：运行 \`svp prompt <action> <id>\` 获取提示词 → 读取 prompt 头部 complexity 字段 → 派发 subagent → 后续跑 toolchain 命令。
...
- JSON 中 contentHash 和 revision 写占位值，\`svp rehash\` 会修正
- 尽量并行派发无依赖的 subagent
...
**Subagent dispatch**: Run \`svp prompt <action> <id>\` to get the prompt → read the complexity field in the prompt header → dispatch subagent → then run toolchain commands.
...
- Write placeholder values for contentHash and revision in JSON; \`svp rehash\` will fix them
...
export function defaultSlashCommands(language: string, command = "/svp"): SlashCommandEntry[] {
  return [
    {
      command,
      description:
...
export function genericContextOptions(language: string, command = "/svp"): ContextOptions {
  return {
...
- 运行 \`svp check --json\`（忽略错误）+ \`svp view l5\` + 检查 .svp/ 是否存在
- 根据结果判断：
  - **无 .svp/**：告知用户先运行 \`svp init\`，停止
...
**HASH_MISMATCH**
- [Toolchain] 运行 \`svp rehash\` 修正 hash

**MISSING_L2**
- [AI] 运行 \`svp prompt compile <l3-id>\` → subagent 生成代码
- [Toolchain] 运行 \`svp link <l3-id> --files <paths>\`

**SOURCE_DRIFT**
- [AI] 运行 \`svp prompt recompile <l3-id>\` → subagent 更新代码

**CONTENT_DRIFT**
- [AI] 运行 \`svp prompt review <l3-id>\` → subagent 判断：
  - L3 需要更新？还是 L1 需要修复？
  - 向用户展示分析结果

**MISSING_BLOCK_REF**
- [AI] 运行 \`svp prompt update-ref <l4-id>\` → subagent 判断：
...
- 运行 \`svp view l5\` + \`svp view l4\` + \`svp view l3\` 展示完整系统结构
- 如有 L2 映射，也展示 \`svp view l2\`

$ARGUMENTS`;

const workflowEn = `## Step 0: Diagnostic Router

- Run \`svp check --json\` (ignore errors) + \`svp view l5\` + check whether .svp/ exists
- Based on the result, determine:
  - **No .svp/**: Tell user to run \`svp init\` first, then stop
...
- Run \`svp prompt design-l5 --intent "<user intent>"\`
- Dispatch stdout output to subagent (read complexity to select model tier)
- Subagent outputs L5 JSON → write to .svp/l5.json
- [Toolchain] Run \`svp rehash l5\`
- Show \`svp view l5\` to user for confirmation

### Step 2: [AI] Design L4 Artifacts
Choose L4 variant based on system type:
- **Flow** (default): Request-response pipeline → \`svp prompt design-l4 --intent "..."\`
- **EventGraph**: Event-driven/CRDT → \`svp prompt design-l4 --kind event-graph --intent "..."\`
- **StateMachine**: Entity lifecycle → \`svp prompt design-l4 --kind state-machine --intent "..."\`

- Dispatch stdout output to subagent (read complexity to select model tier)
- Subagent outputs L4 JSON → write to .svp/l4/<id>.json
- [Toolchain] Run \`svp rehash l4\`
- Show \`svp view l4\` to user for confirmation

### Step 3: [AI] Design L3 Contracts (dispatch in parallel)
For each blockRef in L4 steps:
- Run \`svp prompt design-l3 <block-id> --flow <flow-id> --step <idx> --intent "..."\`
- Dispatch stdout output to subagent (read complexity to select model tier)
- Subagent outputs L3 JSON → write to .svp/l3/<id>.json
- [Toolchain] Run \`svp rehash l3/<id>\`
- **Dispatch independent blocks in parallel**

### Step 4: [Toolchain] Get Compile Tasks
- Run \`svp compile-plan\` to get the compile task list

### Step 5: [AI] Compile L1 Code (dispatch in parallel)
For each compile task:
- Run \`svp prompt compile <l3-id>\`
- Dispatch stdout output to subagent (read complexity to select model tier)
- Subagent generates src/<id>.ts code file
- **Dispatch independent tasks in parallel**

### Step 6: [Toolchain] Create L2 Mappings
- For each generated file run \`svp link <l3-id> --files src/<id>.ts\`

### Step 7: [Toolchain] Verify
- Run \`svp check\` to validate all layer consistency
...
- Run \`svp view l5\` and \`svp view l4/<id>\` to understand the existing architecture
...
- [Toolchain] Run \`svp rehash l4\`
- Show \`svp view l4/<flow-id>\` to user for confirmation

### Step 3: [AI] Design New L3 Contract
- Run \`svp prompt design-l3 <new-block-id> --flow <fid> --step <idx> --intent "..."\`
- Dispatch stdout output to subagent (read complexity to select model tier)
- Subagent creates .svp/l3/<id>.json
- [Toolchain] Run \`svp rehash l3/<id>\`

### Step 4: [AI] Compile New Code
- Run \`svp prompt compile <new-block-id>\`
- Dispatch stdout output to subagent (read complexity to select model tier)
- Subagent generates L1 source code

### Step 5: [Toolchain] Create Mapping and Verify
- \`svp link <l3-id> --files <paths>\`
- \`svp check\` to confirm all green
...
- Run \`svp check\` to confirm current consistency state
- Run \`svp view l5\` + \`svp view l4\` + \`svp view l3\` to understand the structure
...
- L5 change: edit .svp/l5.json → \`svp rehash l5\`
- L4 change: edit .svp/l4/<id>.json → \`svp rehash l4\`
- L3 change: edit .svp/l3/<id>.json → \`svp rehash l3/<id>\`
- Show to user for confirmation

### Step 4: [Toolchain] Get Affected Tasks
- Run \`svp compile-plan\` to get the recompile task list for affected entities

### Step 5: [AI] Recompile Affected Code
For each recompile task:
- Run \`svp prompt recompile <l3-id>\`
- Dispatch stdout output to subagent (read complexity to select model tier)
...
- Run \`svp check --json\` to get the structured issue list

### Step 2: Handle by issueCode Category

**HASH_MISMATCH**
- [Toolchain] Run \`svp rehash\` to fix hash

**MISSING_L2**
- [AI] Run \`svp prompt compile <l3-id>\` → subagent generates code
- [Toolchain] Run \`svp link <l3-id> --files <paths>\`

**SOURCE_DRIFT**
- [AI] Run \`svp prompt recompile <l3-id>\` → subagent updates code

**CONTENT_DRIFT**
- [AI] Run \`svp prompt review <l3-id>\` → subagent determines:
  - Does L3 need updating? Or does L1 need fixing?
  - Show the analysis results to the user

**MISSING_BLOCK_REF**
- [AI] Run \`svp prompt update-ref <l4-id>\` → subagent determines:
...
- Re-run \`svp check\` to confirm fixes are effective
...
- Run \`svp view l5\` + \`svp view l4\` + \`svp view l3\` to show full system structure
- If L2 mappings exist, also show \`svp view l2\`
...
4. Subagent 完成后运行 svp link / svp rehash / svp check
\`\`\`
...
| 命令 | 说明 |
|---|---|
| \`svp view l5/l4/l3/l2\` | 以 AI 友好格式查看层数据 |
| \`svp check\` | 校验跨层一致性 |
| \`svp compile-plan\` | 检测变更并生成重编译任务 |
| \`svp rehash [target]\` | 重算 contentHash + 递增 revision |
| \`svp link <l3-id> --files <paths>\` | 创建/更新 L2 code block 映射 |
| \`svp prompt <action> <id>\` | 生成上下文感知的 AI 提示词到 stdout |

### Prompt 命令

| 命令 | 说明 |
|---|---|
| \`svp prompt compile <l3-id>\` | 生成 L3→L1 编译提示词 |
| \`svp prompt recompile <l3-id>\` | 生成重编译提示词（L3 已变更） |
| \`svp prompt review <l3-id>\` | 生成审查提示词（L1 漂移） |
| \`svp prompt update-ref <l4-id>\` | 生成修复断裂 L4→L3 引用的提示词 |
| \`svp prompt design-l5 --intent "..."\` | 生成 L5 设计提示词 |
| \`svp prompt design-l4 --intent "..." [--kind flow|event-graph|state-machine]\` | 生成 L4 设计提示词 |
| \`svp prompt design-l3 <id> --flow <fid> --step <n> --intent "..."\` | 生成 L3 设计提示词 |
...
3. **Hash 管理**：在 JSON 中写 \`"placeholder"\` 作为 contentHash。运行 \`svp rehash\` 修正。
4. **L2 创建**：生成 L1 代码后，运行 \`svp link <l3-id> --files <paths>\` 创建 L2 映射。
5. **验证**：每层完成后运行 \`svp check\` 确保一致性。
...
|-------|-----------|--------|
| **AI** | Design L5/L4/L3 specs | \`svp prompt design-*\` → subagent |
| **AI** | Compile L3→L1 code | \`svp prompt compile/recompile\` → subagent |
| **AI** | Review drift | \`svp prompt review\` → subagent |
| **AI** | Fix broken references | \`svp prompt update-ref\` → subagent |
| **Toolchain** | Validate consistency | \`svp check\` |
| **Toolchain** | Render layer views | \`svp view\` |
| **Toolchain** | Generate compile task list | \`svp compile-plan\` |
| **Toolchain** | Create/update L2 mapping | \`svp link\` |
| **Toolchain** | Recompute hash | \`svp rehash\` |
...
Toolchain operations run CLI directly: \`svp check\`, \`svp view l3\`, etc.
