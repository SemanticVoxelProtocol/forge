...
export function registerFix(program: Command): void {
  program
    ...
    .option(
      "--action <type>",
      "Only generate prompts for this action (compile/recompile/update-ref/review)",
    )
    .option("--dry-run", "Print compile-plan summary only, do not generate prompts")
    .action(
      async (options: { root: string; severity: string; action?: string; dryRun?: boolean }) => {
        ...
        if (options.dryRun === true) {
          ...
          for (const [i, task] of tasks.entries()) {
            console.log(
              `  [${String(i + 1)}] ${task.action} · ${task.targetLayer}/${task.targetId} · ${task.issueCode}`,
            );
          }
          return;
        }
        ...
        for (const [i, task] of tasks.entries()) {
          if (i > 0) console.log("\n---\n");
          console.log(
            `## [${String(i + 1)}/${String(tasks.length)}] ${task.action} · ${task.targetLayer}/${task.targetId} · ${task.issueCode}\n`,
          );
